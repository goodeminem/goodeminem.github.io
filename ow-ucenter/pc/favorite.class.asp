...
%>
    ...
    <div id="mbody"> 
      <div id="container">
          <h1 class="header"><%=UC.lang(1200)%></h1>
          <div class="section-favorite" id="favorite_goods">
              <div class="section">
				  ...
				  OW.Pager.run()
				  %>
                  <ul class="goods-list" name="goods">
                  <%=OW.Pager.loopHtmls%>
                  </ul>
              </div>
    ...
    $(window).ready(function(){
		...
		//积分价格
		$(".price").each(function(){
			var price = OW.parseMoney($(this).find(".money > b").html());
			if(!price>0){
		...
		$favorites.find("a[name='delete']").click(function(){
			...
			$dialog = new OWDialog({
				...
				ok:function(){
					...
					data    = "gid="+gid;
					$dialog.button({id:"ok",remove:true},{id:"cancel",remove:true}).posting();
					OW.ajax({
						url:url,data:data,
						success:function(){
