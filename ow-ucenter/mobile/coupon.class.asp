...
class UC_COUPON_CLASS
	...
	call UC.echoHeader()
%>
	<%=UC.htmlHeaderMobile("<a href="""& UCENTER_URL &""" class=""ucenter""></a>",UC.lang(1100))%>
    <section id="mbody">
        <div class="section-coupon" id="coupons">
            <div class="header">
                <ul><li><a <%=OW.iif(V("status")="valid","class=""current""","")%> href="<%=UCENTER_HURL%>ctl=coupon&status=valid"><%=UC.lang(1101)%></a></li><li><a <%=OW.iif(V("status")="used","class=""current""","")%> href="<%=UCENTER_HURL%>ctl=coupon&status=used"><%=UC.lang(1102)%></a></li><li class="li-3"><a <%=OW.iif(V("status")="invalid","class=""current""","")%> href="<%=UCENTER_HURL%>ctl=coupon&status=invalid"><%=UC.lang(1103)%></a></li></ul>
            </div>
            <div class="section">
                <%
                ...
                OW.Pager.loopHtml = "<div class=""coupon coupon-"& V("status") &"""><dl><dt><em class=""moneysb"">￥</em><span class=""money"">{$coupon_money}</span><span class=""coupon-name"">"& UC.lang(1104) &"</span></dt><dd><div class=""use-time"" name=""use_time"" is_use_timelimit=""{$is_use_timelimit}""><div class=""span-dt"">"& UC.lang(1105) &"</div><div class=""span-dd"" name=""use_time_show"">{$use_starttime} 至 {$use_endtime}</div></div><div class=""use-min-money"" use_min_money=""{$use_min_money}""><div class=""span-dt"">"& UC.lang(1106) &"</div><div class=""span-dd"">"& UC.lang(1107) &"</div></div></dd></dl></div>"
                OW.Pager.run()
                %>
                <%=OW.Pager.loopHtmls%>
            </div>
